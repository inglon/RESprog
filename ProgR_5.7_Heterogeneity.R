################################################################
################################################################
# Script: ProgR_5.7_Heterogeneity.R
...
DoHapseg2 <- function(myarray, mysample) {
  ...
  snp.fn <- file.path(d, 'snpfile.RData')
  seg.fn = file.path(d, 'Segdat.txt')
  results.dir <- file.path(getwd(), 'ABSOLUTE', 'WithNormals', 'HAPSEG2', mysample)
  if (!file.exists(results.dir)) {
    dir.create(results.dir, recursive=TRUE)
  }
  print(paste("Starting array", mysample, "at", results.dir))
  log.dir <- file.path(getwd(), 'ABSOLUTE', 'WithNormals', 'HAPSEG2', 'log')
  if (!file.exists(log.dir)) {
    dir.create(log.dir, recursive=TRUE)
  }
  sink(file=file.path(log.dir, paste(mysample, ".hapseg.out.txt", sep="")))
...
##### Read complete mutations file from which we need read depths  for variant and reference
#####
#####
d = file.path(getwd(), 'AROMA', 'rawData', 'responsify', 'Exome', "Data3AllVariants.tsv")
dat = read.delim(d)
dat = data.frame(Tumor_Sample_Barcode = paste(dat$Sample, 'T', sep = ''),
                 Chromosome = as.character(dat$X.CHROM), Start_position = dat$POS, 
                 t_alt_count = as.character(dat$Tumor.AD),
                 t_ref_count = as.numeric(as.character(dat$Tumor.RD)), 
                 ref = as.character(dat$REF), alt = as.character(dat$ALT), 
                 stringsAsFactors = F) #Warnings are due OK
tmp = dat$t_alt_count
tmp[tmp == '.'] = NA
tmp = gsub('|', ',', x = tmp, fixed = T)
tmp = unlist(lapply(strsplit(tmp, split = ',', fixed = T), function(x){sum(as.numeric(x))}))
dat$t_alt_count = tmp
dat = unique(dat)
dat$uni = paste(dat$Tumor_Sample_Barcode, dat$Chromosome, dat$Start_position, dat$ref,
                       dat$alt, sep = '.')
tab = table(dat$uni)
dups = names(tab[tab>1])
subset(dat, uni %in% dups) #Should have 0 rows of duplicates left

##### Use Oncotator annotated mutations for heterogeneity
##### 
##### 

d = file.path(getwd(), 'AROMA', 'rawData', 'responsify','Exome', 'Mutsig_non-silent')
maf = read.delim(file.path(d,"AllVarintsMutectOncotatorPlusMutsigCategNo5214.tsv"))
maf$Tumor_Sample_Barcode = substr(as.character(maf$Tumor_Sample_Barcode), 2, 6)
maf$ref = as.character(maf$ref_allele)
maf$alt = as.character(maf$Tumor_Seq_Allele1)
##### Add additional required variables from dat to maf and 
# filter mutations with >=10 reads or non-paired reads
nrow(maf)
maf = merge(maf, dat, all.x = T, sort = F)
nrow(maf)
#[1] 8930
maf = subset(maf, maf$t_alt_count + maf$t_ref_count >=10 & !is.na(maf$t_alt_count) & 
               !is.na(maf$t_ref_count))
nrow(maf)
#[1] 8795
...
mafbase = "AllVarintsMutectOncotatorPlusMutsigCategNo5214_mindepth10_withvaf"
for (i in 6:56){
  samp = info$sample[i]
...
DoAbsolute <- function(myarray, mysample, i) {
  ...
  maf.fn = paste(getwd(), 
                 '/AROMA/rawData/responsify/Exome/MAF/AllVarintsMutectOncotatorPlusMutsigCategNo5214_mindepth10_withvaf', 
                 '_', 
                 mysample, '.maf', sep = '')
  h = 2
...
foreach (i = inds, .combine=c) %dopar% {
  DoAbsolute(myarray = info$myarray[i], mysample = paste(i,info$sample[i], sep = '.'), i=i)
}
